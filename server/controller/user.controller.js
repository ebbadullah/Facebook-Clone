...
let suggestedUser = async (req, res) => {
  try {
    ...
    if (findUser && findUser.length > 0) return res.status(200).json({ status: true, message: "find suggested user successfully", suggestedUser: findUser })
    return res.status(200).json({ status: true, message: "no suggested users found", suggestedUser: [] })
  } catch (error) {
    console.error("Error in suggestedUser:", error)
...
let followAndUnfollow = async (req, res) => {
  try {
    ...
    await Promise.all([
      user.updateOne({ _id: userMe }, isFollowing ? { $pull: { following: userYou } } : { $push: { following: userYou } }),
      user.updateOne({ _id: userYou }, isFollowing ? { $pull: { followers: userMe } } : { $push: { followers: userMe } }),
    ])
...
let sendFriendRequest = async (req, res) => {
  try {
    ...
    // ensure arrays
    sender.following = Array.isArray(sender.following) ? sender.following : []
    receiver.followers = Array.isArray(receiver.followers) ? receiver.followers : []
    sender.sentFriendRequests = Array.isArray(sender.sentFriendRequests) ? sender.sentFriendRequests : []
    receiver.friendRequests = Array.isArray(receiver.friendRequests) ? receiver.friendRequests : []
...
let getFriendRequests = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: true, data: friendRequests })
  } catch (error) {
...
let blockUser = async (req, res) => {
  try {
    ...
    blocker.blockedUsers = Array.isArray(blocker.blockedUsers) ? blocker.blockedUsers : []
...
let getBlockedUsers = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: true, data: blocked })
  } catch (error) {
...
let getUserFriends = async (req, res) => {
  try {
    ...
    const friends = following.filter(friend => Array.isArray(friend.followers) && friend.followers.includes(userId))
