...
const Header = ({ onMobileMenuToggle }) => {
    ...
    const navigationItems = [
        ...
        { icon: OndemandVideo, label: "Watch", onClick: handleVideoTab },
            ...
            <AppBar position="fixed" elevation={0} sx={{ backgroundColor: "white", borderBottom: "1px solid #e4e6ea", zIndex: 1100 }}>
                <Toolbar sx={{ justifyContent: "space-between", px: { xs: 1, md: 2 }, minHeight: "56px" }}>
                    <Box sx={{ display: "flex", alignItems: "center", flex: { xs: 1, md: 1 } }}>
                        <IconButton sx={{ display: { xs: "flex", lg: "none" }, mr: 1, color: "#1c1e21" }} onClick={onMobileMenuToggle}>
                            <MenuIcon />
                        </IconButton>

                        <Typography variant="h4" sx={{ fontWeight: "bold", color: "#1877f2", cursor: "pointer", mr: { xs: 1, md: 2 }, fontSize: { xs: "1.2rem", sm: "1.5rem", md: "2rem" } }} onClick={() => navigate("/")}>
                            facebook
                        </Typography>
                    ...
                    <Box sx={{ display: "flex", alignItems: "center", flex: { xs: 0, md: 1 }, justifyContent: "flex-end", gap: { xs: 0.5, sm: 1 } }}>
                        <IconButton sx={{ display: { xs: "flex", md: "none" }, backgroundColor: "#f0f2f5", width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 }, "&:hover": { backgroundColor: "#e4e6ea" } }} onClick={() => setMobileSearchOpen(true)}>
                            <Search sx={{ color: "#1c1e21", fontSize: "20px" }} />
                        </IconButton>
                        ...
                        <IconButton sx={{ backgroundColor: "#f0f2f5", width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 }, transition: "all 0.2s ease", "&:hover": { backgroundColor: "#e4e6ea", transform: "scale(1.05)" } }}>
                            <Badge badgeContent={3} color="error" sx={{ "& .MuiBadge-badge": { fontSize: "11px", height: "18px", minWidth: "18px" } }}>
                                <Message sx={{ color: "#1c1e21", fontSize: "20px" }} />
                            </Badge>
                        </IconButton>
                        ...
                        <IconButton onClick={handleMenuOpen} sx={{ ml: 0.5, p: 0.5, borderRadius: "50px", transition: "all 0.2s ease", "&:hover": { backgroundColor: "#f0f2f5" } }}>
                            <Avatar src={user?.ProfilePicture} sx={{ width: { xs: 28, sm: 32 }, height: { xs: 28, sm: 32 }, border: activeTab !== null ? "2px solid #1877f2" : "2px solid transparent", transition: "border-color 0.2s ease" }}>
                                {user?.name?.charAt(0)}
                            </Avatar>
                        ...
                        <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose} PaperProps={{ sx: { mt: 1, minWidth: { xs: 260, sm: 280 }, borderRadius: "8px", boxShadow: "0 8px 16px rgba(0,0,0,0.15)", border: "1px solid #e4e6ea", "& .MuiMenuItem-root": { px: 2, py: 1.5, borderRadius: "6px", mx: 1, my: 0.5, transition: "background-color 0.2s ease", "&:hover": { backgroundColor: "#f0f2f5" } } } }}>
                            <MenuItem onClick={handleProfile}>
                                <Avatar src={user?.ProfilePicture} sx={{ width: 36, height: 36, mr: 2 }}>
                                    {user?.name?.charAt(0)}
                                </Avatar>
                                <Box>
